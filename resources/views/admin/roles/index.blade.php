                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="text-white-50 mb-0">{{ __('Manage Roles') }}</p>
                        </div>
                        <div>
                            <button class="btn modern-btn-primary" id="addRole" data-bs-toggle="modal" data-bs-target="#addModal">
...
@push('scripts')
    {!! $dataTable->scripts() !!}
    <script src="{{ asset('admin/ajax/roles.js?v=' . time()) }}"></script>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#table_filter input').attr('placeholder', '{{ __("Search Roles") }}...');
...
@if(auth()->user()->hasRole('admin') || auth()->user()->hasPermission('role-add'))
@include('admin.roles.create')
@endif
@if(auth()->user()->hasRole('admin') || auth()->user()->hasPermission('role-update'))
@include('admin.roles.edit')
@endif
@if(auth()->user()->hasRole('admin') || auth()->user()->hasPermission('role-permission'))
